...
class WebosTv extends EventEmitter {
  constructor(opts = {}) {
    ...
    this._uiLanguage = null; // menu locale, read lazily; see uiLanguage()
  }
  ...
  _handleDisconnect() {
    ...
    this.connected = false;
    this.emit('close');
    // Re-read the menu language on the next write: cheap, and it means changing
    // the TV's language takes effect on reconnect instead of needing a redeploy.
  ...
  /**
   * The TV's menu language as a locale tag (`pt-PT`), so the alert above can be
   * labelled in the language the viewer actually reads.
   *
   ...
   * so switching the TV's language self-heals on the next reconnect instead of
   * needing a redeploy.
   *
   * Best-effort by design: returns null on any failure, so a locale problem can
   * never turn a working write into a failed one.
   */
  async uiLanguage() {
    if (this._uiLanguage) {
    ...
    try {
      const res = await this.getSystemSettings('option', ['localeInfo']);
      const locales = res && res.settings && res.settings.localeInfo && res.settings.localeInfo.locales;
      const tag = locales && locales.UI;
      if (typeof tag === 'string' && tag) {
        this._uiLanguage = tag;
        return tag;
      }
      this.log.debug(`webOS TV "${this.name}": no UI locale in localeInfo, labelling in English`);
    } catch (err) {
      this.log.debug(`webOS TV "${this.name}": could not read UI language (${err.message}), labelling in English`);
    }
    return null;
  }
