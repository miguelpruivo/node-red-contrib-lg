...
class WebosTv extends EventEmitter {
   ...
   *  - The alert may flash briefly on screen on some models.
   */
  async lunaSend(uri, params = {}) {
  ...
  setSystemSettings(category, settings) {
    return this.lunaSend(URI_LUNA_SET_SETTINGS, { category, settings });
  }
  ...
  setPictureSettings(settings) {
    return this.setSystemSettings('picture', settings);
  }
