...
class WebosTv extends EventEmitter {
  constructor(opts = {}) {
    ...
    // The secure setting the user configured, and the one we have actually
    // completed a pairing handshake on. A watchdog restart reverts an unproven
    // ws->wss fallback so one stray ECONNRESET cannot strand the node on a port
    // the TV does not serve.
    this._configuredSecure = this.secure;
  ...
  _checkStalled() {
    ...
    if (Date.now() - this._lastActivityAt < this.watchdogMs) {
      return; // still cycling connecting -> failure: retrying normally, not stuck
    }
    this.log.debug(`webOS TV "${this.name}": no connection activity for ${this.watchdogMs}ms, rebuilding the client`);
    this._restart({ resetTransport: true });
  }
  ...
  /**
   * Rebuild the lgtv2 client. `resetTransport` is for the watchdog only: when
   * nothing has worked for a whole window, drop an unproven ws->wss fallback and
   * start over from the configured transport. A transport that has completed a
   * handshake is kept — it is known good.
   *
   * The ws->wss fallback itself restarts *without* the flag, since resetting
   * there would immediately undo the flip it is trying to apply and loop on a
   * port the TV refuses.
   */
  _restart(opts = {}) {
    this._teardownClient();
    this.connected = false;
    if (opts.resetTransport && this._provenSecure === null) {
      this.secure = this._configuredSecure;
    }
    this._noteActivity();
    this._setup();
  }
