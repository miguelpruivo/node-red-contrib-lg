...
const URI_GET_SETTINGS = 'ssap://settings/getSystemSettings';
const URI_CREATE_ALERT = 'ssap://system.notifications/createAlert';
const URI_CLOSE_ALERT = 'ssap://system.notifications/closeAlert';
const URI_LUNA_SET_SETTINGS = 'luna://com.webos.settingsservice/setSystemSettings';
...
class WebosTv extends EventEmitter {
  constructor(opts = {}) {
    ...
    this._uiCountry = null; // read lazily; see uiCountry()
  }
  ...
  _handleDisconnect() {
    ...
    this.connected = false;
    this.emit('close');
    // Re-read the country on the next write: cheap, and it means a change
    // takes effect on reconnect instead of needing a redeploy.
    this._uiCountry = null;
  ...
  /**
   * Read system settings. Reads are permitted for a paired client, so this is a
   * plain SSAP request -- only *writes* need the alert route below.
   */
  getSystemSettings(category, keys) {
    ...
    return this.request(URI_GET_SETTINGS, payload);
  }

  /**
   * Run a luna:// service call.
   *
   * LG does not let a paired third-party client *write* settings: the obvious
   * `ssap://settings/setSystemSettings` is refused on consumer firmware ("no
   * such service or method") even though lgtv2's pairing handshake asks for
   * WRITE_SETTINGS. The route every project uses instead (homebridge-webos-tv,
   * bscpylgtv, the Home Assistant scripts) is to open a *system* alert whose
   * action is the luna call and then close it: the notification manager runs
   * that action with its own privileges.
   *
   * Details that matter:
   *  - `isSysReq: true` is what makes it privileged. Without it, nothing happens.
   *  - Closing the alert is what fires the call, so it is repeated in `onclose`
   *    (and `onfail`), not just on the button.
   *  - webOS < 4 does not run it from closeAlert (those need the alert opened
   *    twice plus an ENTER on the remote-input socket). Not supported here.
   *
   * The alert is *visible* on some models -- `message: ' '` is an attempt to
   * hide it that does not work everywhere. Callers that know what they are
   * writing should pass a `message` describing it, so what appears on screen
   * reads like the TV talking rather than an empty box.
   *
   * `holdMs` keeps the alert up before closing it. Because closing is what
   * executes the call, a hold delays the write by exactly that long -- it buys
   * on-screen readability with latency, so it defaults to 0 (close at once, the
   * historical behaviour) and only the picture path opts in.
   */
  async lunaSend(uri, params = {}, { message = ' ', holdMs = 0 } = {}) {
    const action = { uri, params };
    const res = await this.request(URI_CREATE_ALERT, {
      message,
      modal: false,
      type: 'confirm',
      isSysReq: true,
      buttons: [{ label: 'ok', focus: true, buttonType: 'ok', onClick: uri, params }],
      onclose: action,
      onfail: action,
    });
    const alertId = res && res.alertId;
    if (!alertId) {
      throw new Error(`TV returned no alertId for ${uri} -- this firmware will not run luna calls`);
    }
    if (holdMs > 0) {
      // The alert carries a focused `ok` button while it is up. Pressing that on
      // the remote inside the window fires `onClick` and closes the alert, and
      // the closeAlert below then fires `onclose` -- so the same values get
      // written twice. Harmless (the write is idempotent), so it is not guarded.
      await sleep(holdMs);
    }
    await this.request(URI_CLOSE_ALERT, { alertId });
    return { alertId };
  }

  setSystemSettings(category, settings, options) {
    return this.lunaSend(URI_LUNA_SET_SETTINGS, { category, settings }, options);
  }

  /**
   * The TV's country as an ISO-3166 alpha-3 code (`PRT`), used to guess which
   * language to label the alert above in.
   *
   * Country and not menu language on purpose: **the language is not readable**.
   * Measured live on webOS 7.0, `localeInfo` is refused -- "Some keys are not
   * allowed for the request. ( localeInfo )" -- under every category tried, as
   * is every other language-ish key, and the `com.webos.settingsservice` URI
   * answers `{}` to everything. `country` is allowed. An earlier version asked
   * for `localeInfo` and therefore fell back to English on every single write.
   *
   * A plain SSAP read -- no alert needed. Cached, because it is wanted on every
   * picture write and changes about never; `_handleDisconnect` clears the cache
   * so a change self-heals on the next reconnect instead of needing a redeploy.
   *
   * Best-effort by design: returns null on any failure, so this can never turn
   * a working write into a failed one.
   */
  async uiCountry() {
    if (this._uiCountry) {
      return this._uiCountry;
    }
    try {
      const res = await this.getSystemSettings('option', ['country']);
      const code = res && res.settings && res.settings.country;
      if (typeof code === 'string' && code) {
        this._uiCountry = code;
        return code;
      }
      this.log.debug(`webOS TV "${this.name}": no country reported, labelling in English`);
    } catch (err) {
      this.log.debug(`webOS TV "${this.name}": could not read country (${err.message}), labelling in English`);
    }
    return null;
  }

  /**
   * Picture settings. On an OLED panel `backlight` (0-100) is "OLED Pixel
   * Brightness"; `brightness` in the same category is Black Level, NOT the
   * panel light. Values are stored per picture preset (and separately for
   * SDR/HDR), so pin `pictureMode` in the same call when it must be
   * deterministic. Energy Saving can clamp or override the value.
   */
  setPictureSettings(settings, options) {
    return this.setSystemSettings('picture', settings, options);
  }
