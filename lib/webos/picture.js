...
/**
 * Reduce a language or locale tag (`pt`, `pt-PT`, `en_US`) to a language we
 * have copy for. Anything unknown, missing or malformed becomes English --
 * language detection is best-effort and must never be able to break a write.
 */
...
/**
 * Country -> language, because **the TV will not report its menu language**.
 *
 * Measured live on webOS 7.0 (`HE_DTV_W22O_AFABATPU`, firmware 33.31.68):
 * `localeInfo` is refused outright -- "Some keys are not allowed for the
 * request. ( localeInfo )" -- under categories `option`, `general`, `locale`
 * and `localeInfo`, and `ssap://com.webos.settingsservice/getSystemSettings`
 * answers `{}` to everything. Every language-ish key (`audioLanguage`,
 * `subtitleLanguage`, `language`, `languageCode`, `locale`) is refused too. The
 * readable keys are `country` ("PRT") and `smartServiceCountryCode3`.
 *
 * So this is a **guess from the country**, not the menu language: a set in
 * Portugal switched to English menus still gets Portuguese copy. That is why
 * the node has an explicit Language option to override it, and why the guess
 * only has to be right often enough to beat always-English.
 */
const COUNTRY_LANGUAGE = {
  PRT: 'pt', BRA: 'pt',
  ESP: 'es', MEX: 'es', ARG: 'es', CHL: 'es', COL: 'es', PER: 'es',
  FRA: 'fr',
  NLD: 'nl', BEL: 'nl',
  DEU: 'de', AUT: 'de', CHE: 'de',
  ITA: 'it',
  POL: 'pl',
  SWE: 'sv', DNK: 'da', NOR: 'nb', FIN: 'fi',
  TUR: 'tr', RUS: 'ru',
};
