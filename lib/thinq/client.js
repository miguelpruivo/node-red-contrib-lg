...
class ThinQClient {
  ...
  async _request(method, uri, data, retry = false) {
    ...
    try {
      ...
      const code = err.response && err.response.data && err.response.data.resultCode;
  ...
  async sendCommand(deviceId, dataKey, dataValue, opts = {}) {
    ...
    const resp = await this._request('post', `service/devices/${deviceId}/${ctrlPath}`, body);
    return resp;
  }

  /**
   * Send several { dataKey, dataValue } commands sequentially.
   */
