...
function parseSnapshot(snapshot) {
  ...
  return {
    ...
    // Panel display LED, reported as a friendly boolean (true = lit). The raw
    // displayControl key is inverted: 0 = on, 1 = off (11 = off on other models).
    display: displayIsOn(num(snapshot, KEYS.DISPLAY)),
  };
}

/**
 * Map a raw airState.lightingState.displayControl value to a boolean (true =
 * display lit). Inverted: 0/12/13 = on, 1/11 = off. null when not reported.
 */
function displayIsOn(value) {
  if (value === null) {
    return null;
  }
...
function buildCommands(request) {
  ...
  if ('fan' in request && request.fan !== undefined && request.fan !== null) {
    ...
    if (fan === null) {
      throw new Error(
        ...
        `MID_HIGH, HIGH, POWER, AUTO, or a numeric value (supported speeds vary by model).`
      );
    }
  ...
  // Panel display LED: { display: true|false } (also "on"/"off"). The device key
  // is inverted (0 = on, 1 = off) and only takes effect while the unit is on, so
  // it flows through flush() as an on-only setting like mode/temperature.
  if ('display' in request && request.display !== undefined && request.display !== null) {
    const on = resolvePower(request.display);
    if (on === null) {
      throw new Error(`Invalid display value: ${request.display}`);
    }
    commands.push({ dataKey: KEYS.DISPLAY, dataValue: on === 1 ? 0 : 1, label: `display=${on === 1 ? 'on' : 'off'}` });
  }
